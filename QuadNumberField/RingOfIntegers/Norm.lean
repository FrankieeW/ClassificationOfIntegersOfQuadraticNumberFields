...
-/
import QuadNumberField.RingOfIntegers.Classification
...
theorem norm_mem_ringOfIntegers (d : ℤ) [QuadFieldParam d]
    (α : 𝓞 (QuadNumberField d)) :
    ∃ n : ℤ, Qsqrtd.norm (α : QuadNumberField d) = n := by
  rcases ringOfIntegers_classification d with ⟨hd4, h_equiv⟩ | ⟨k, hk, h_equiv⟩
  · -- d % 4 ≠ 1 branch: 𝓞 ≃ ℤ[√d]
    let e := Classical.choice h_equiv
    let z : Zsqrtd d := e α
    -- TODO: Prove that (α : QuadNumberField d) = Zsqrtd.toQsqrtd z
  ...
  · -- d % 4 = 1 branch: 𝓞 ≃ ℤ[(1 + √d)/2]
    ...
    -- TODO: Prove that (α : QuadNumberField (1 + 4 * k)) = toQsqrtdHom k z
    sorry
